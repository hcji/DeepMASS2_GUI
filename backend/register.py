...
from backend.app import demo as io
from backend.entity.user import User, Code
from backend.utils.send_email import gen_random_code, send_mail_deepmass
...
async def register(
    ...
    # 将用用户输入的验证码与数据库里的进行对比，正确则继续后面的注册流程，错误则提示用户
    # 查询验证码是否存在
    rescode = (
        session.execute(select(Code).where(Code.contact_info.in_([contact_info])))
        .scalars()
        .one_or_none()
    )
    ...
    if rescode.verify_code == vercode:
        ...
        obj = session.execute(select(User).where(User.contact_info.in_([contact_info])))
