...
model = load_model(os.path.join(path_data, 'MS2DeepScore_allGNPSnegative.hdf5'))
...
p.add_items(xb, ids)
p.set_ef(300)
p.save_index('data/references_index_negative.bin')
np.save('data/references_spectrums_negative.npy', reference)
...
p_n.set_ef(300)
p_n.save_index('data/references_index_negative_new.bin')
np.save('data/references_spectrums_negative_new.npy', reference[np.array(k)])
