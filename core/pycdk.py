...
def FormulaFromString(string):
    ...
    formula = cdk.tools.manipulator.MolecularFormulaManipulator.getMolecularFormula(
        string, builder
    )
    return formula
...
def getFingerprint(mol, fp_type="standard", size=1024, depth=6, transform=True):
    if fp_type == "maccs":
        ...
        nbit = size
    _fingerprinters = {
        "standard": cdk.fingerprint.Fingerprinter(size, depth),
...
def generateFragments(
    mol,
    method="MurckoFragmenter",
    minFragSize=6,
    singleFrameworkOnly=True,
    asSmiles=True,
):
...
def getMolecularDescriptorCategories():
    function = cdk.qsar.DescriptorEngine(
        cdk.qsar.IMolecularDescriptor, cdk.silent.SilentChemObjectBuilder.getInstance()
    )
